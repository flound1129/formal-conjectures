...
/-- A finite forbidden graph, carrying its own finite vertex set. -/
...
/--
The Erdős quantity from the problem statement: the minimal number of edges in a graph on $n$
vertices such that every subset of $k+2$ vertices induces a subgraph of maximum degree at
least $k$.
-/
...
/--
Let $f(n,k)$ be minimal such that there is a graph with $n$ vertices and $f(n,k)$ edges where
every set of $k+2$ vertices induces a subgraph with maximum degree at least $k$. Determine
$f(n,k)$.
-/
...
For each fixed $k$, there exists a finite family of graphs $F_k^{\min}$, depending only on $k$,
such that
$$f(n,k) = \binom{n}{2} - \mathrm{ex}(n, F_k^{\min}).$$

This is a proven equivalence (see the Ma–Tang note linked above) that re-expresses the open
problem as a Turán-type extremal question. The original problem remains open: determining
$f(n,k)$ is equivalent to determining the extremal numbers $\mathrm{ex}(n, F_k^{\min})$.
-/
